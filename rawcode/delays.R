...
# Prints a message
PrintMessage <-function(filename) {
  msg <- paste0('Creating canvas: ', filename)
  print(msg)
}
...
delayByMonthOverylaid<-function(years) {
  ...
  filename <- paste0('figures/ArrivalDelayByMonth.pdf')
  ...
  for (yr in years) {
    if (i==2) {
      plot(x=df.arr$Month, y=df.arr[,i], type='l', col=yr, 
           main='Mean Arrival Delay by Month', 
           xlab='Month', ylab='Mean (Minutes)', 
           ylim=c(-2,30),pch=pch.all, lwd=lwd.all)
    } else {
      lines(df.arr[,i], col=yr, lwd=lwd.all)
    }
    i <- i + 1
  }
  legend("topleft", legend=years, pch=pch.all, col=years,border="white", lwd=lwd.all,box.lwd = 0)
  
  dev.off()

  # Departure delay
  filename <- paste0('figures/DepartureDelayByMonth.pdf')
  PrintMessage(filename)
  pdf(filename)
  i <- 2
  leg.text <- list()
  for (yr in years) {
    if (i==2) {
      plot(x=df.dep$Month, y=df.dep[,i], type='l', col=yr, 
           main='Mean Departure Delay by Month', 
           xlab='Month', ylab='Mean (Minutes)', 
           ylim=c(-2,30),pch=pch.all, lwd=lwd.all)
    } else {
      lines(df.dep[,i], col=yr, lwd=lwd.all)
    }
    i <- i + 1
  }
  legend("topleft", legend=years, pch=pch.all, col=years,border="white", lwd=lwd.all,box.lwd = 0)
...
# Calculate mean delay for a given airport arrival or
# departure times by month for all years
...
# Airports with most delays in a given year, month
# rank = top to last
MostDelaysByMonth <- function(rank,month, year) {
...
delayByMonthOverylaid(c(2000:2004))